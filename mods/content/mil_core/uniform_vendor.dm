...
/obj/machinery/uniform_vendor/interact(mob/user)
	...
	if(!ID)
		...
		uniforms = find_uniforms(ID.military_rank, ID.military_branch)
...
*/
/obj/machinery/uniform_vendor/proc/find_uniforms(var/datum/mil_rank/user_rank, var/datum/mil_branch/user_branch) //returns 1 if found branch and thus has a base uniform, 2, branch and department, 0 if failed.
	var/decl/hierarchy/mil_uniform/user_outfit = GET_DECL(/decl/hierarchy/mil_uniform)
