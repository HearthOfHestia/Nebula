/obj/structure/solbanner
	name = "\improper ISEO banner"
	icon = 'mods/content/hearth_maps/icons/obj/solbanner.dmi'
	icon_state = "wood"
...
/obj/item/solbanner
	name = "\improper ISEO banner capsule"
	desc = "ISEO banner packed in a rapid deployment capsule. Used for staking claims on new worlds in the name of Sol Central Government."
	icon = 'mods/content/hearth_maps/icons/obj/uniques.dmi'
...
/obj/item/solbanner/attack_self(mob/living/carbon/human/user)
	...
	if(user.unEquip(src))
		...
		if(length(global.using_map.overmap_ids))
			var/obj/effect/overmap/visitable/sector/exoplanet/P = global.overmap_sectors["[z]"]
			if(istype(P))
		...
		E.plantedby = "Planted on [stationdate2text()] by [dudename], [user.get_assignment()] of [global.using_map.full_name]."
