/datum/map/torch
	...
	emergency_shuttle_called_sound = sound('maps/torch/sounds/abandonship.ogg', volume = 45)

	emergency_shuttle_recall_message = "Attention all hands: emergency evacuation sequence aborted. Return to normal operating conditions."

	command_report_sound = sound('maps/torch/sounds/commandreport.ogg', volume = 45)
	...
	grid_check_sound = sound('maps/torch/sounds/poweroff.ogg', volume = 45)

	grid_restored_message = "Ship power to the %STATION_NAME% will be restored at this time"
	grid_restored_sound = sound('maps/torch/sounds/poweron.ogg', volume = 45)

	meteor_detected_sound = sound('maps/torch/sounds/meteors.ogg', volume = 45)
	...
	radiation_detected_sound = sound('maps/torch/sounds/radiation.ogg', volume = 45)

	space_time_anomaly_sound = sound('maps/torch/sounds/spanomalies.ogg', volume = 45)
	...
	unidentified_lifesigns_sound = sound('maps/torch/sounds/aliens.ogg', volume = 45)

	lifesign_spawn_sound = sound('maps/torch/sounds/aliens.ogg', volume = 45)

	electrical_storm_moderate_sound = sound('maps/torch/sounds/electricalstormmoderate.ogg', volume = 45)
	electrical_storm_major_sound = sound('maps/torch/sounds/electricalstormmajor.ogg', volume = 45)

/datum/map/torch/level_x_biohazard_sound(var/bio_level)
	switch(bio_level)
		if(7)
			return sound('maps/torch/sounds/outbreak7.ogg', volume = 45)
		else
			return sound('maps/torch/sounds/outbreak5.ogg', volume = 45)
